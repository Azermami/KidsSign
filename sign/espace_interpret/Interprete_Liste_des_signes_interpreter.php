...
<head>
  ...
  <title>Liste des signes interpretés</title>
...
<a href="Interprete_Liste_des_signes_interpreter.php" class="logo d-flex align-items-center">
...
<li>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Liste des signes interpretés</h1>
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">Liste des signes interpretés</h5>

            <!-- Table with hoverable rows -->
 <?php
include "db_conn.php";
$requete = "SELECT d.id_demande, d.mot, a.video_recu
            FROM demande AS d
            INNER JOIN affectation AS a ON d.id_demande = a.id_demande
            WHERE a.id_interpret = ".$_SESSION['id_interpret'];
            ...
            <div class="col">
                <form action="" method="post">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php while ($row = $reponse->fetch_assoc()) { ?>
                                <tr>
                                    ...
                                    <td>
										...
										<button type="button" class="btn btn-danger btn-sm" onclick="window.location.href='Interpret_Liste_des_nouveaux_mots.php'">Refaire</button>
                                        <button type="submit" class="btn btn-success btn-sm" name="Confirmer" value="<?php echo $row['id_demande']; ?>">Confirmer</button>
                                    </td>
                                </tr>
...
if (isset($_POST['Confirmer'])) {
    ...
    $id_demande = $_POST['Confirmer'];
    
    
    $requeteUpdate = "UPDATE affectation SET etat = 'received' WHERE id_demande = $id_demande";
    $resultatUpdate = $conn->query($requeteUpdate);

    if (!$resultatUpdate) {
        die("Erreur lors de la mise à jour de l'état de la demande : " . mysqli_error($conn));
    }
...
<script>
  ...
  <script src="assets/js/main.js"></script>
   <script>
function showVideo(videoPath) {
    // récupérer la balise vidéo et sa source
