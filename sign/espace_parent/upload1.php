...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["my_image"])) {
 ...
 if ($fileError === 0) {
 $uploadDir = "espace_parent/uploads/";
 ...
 $updatePhotoQuery = "UPDATE parent SET image='$newFileName' WHERE id_parent=$id_parent";
 $updatePhotoResult = mysqli_query($conn, $updatePhotoQuery);



 if ($updatePhotoResult) {
 header('location:Parent_profil.php');
 } else {
...
if (mysqli_num_rows($res) > 0) {
 ...
 $photo = $row['image'];
 if (!empty($photo)) {
 echo '<img src="espace_parent/uploads/' . $photo . '" alt="Profile">';
 } else {
 echo '<img width="120px" height="120px" src="assets/img/test.png" alt="Profile" class="rounded-circle">';
 }
} else {
